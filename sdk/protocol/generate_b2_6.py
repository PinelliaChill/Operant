...
SCHEMA_PATH = ROOT / "sdk/protocol/schema/operant-b2-6.openapi.json"
DIGEST_PATH = ROOT / "sdk/protocol/schema/operant-b2-6.openapi.sha256"
TS_PATH = ROOT / "sdk/typescript-client/b2_6.generated.ts"
PY_PATH = ROOT / "sdk/python_client/b2_6_generated.py"
...
OPERATION_MAP = {
    "negotiateB26": "negotiateProtocol",
    "getB26Experience": "getExperience",
    "getB26Events": "getEvents",
    "executeB26Command": "execute",
    "getB26RemoteQuery": "queryRemote",
}
...
def _document() -> dict[str, Any]:
    ...
    selected["/v1/protocol/b2-6"]["get"]["responses"]["200"]["content"]["application/json"][
        "schema"
...
def _b2_names(source: str) -> str:
    renamed = (
        source.replace("PHASE1E", "B26")
        .replace("Phase1E", "B26")
        .replace("phase1e", "b26")
        .replace("Phase 1E", "B26")
    )
    ...
    return renamed.replace("operant-b26.openapi", "operant-b2-6.openapi").replace(
        "generate_b26.py", "generate_b2_6.py"
    )
...
def generate() -> str:
    ...
    try:
        ...
        python_source = python_source.replace("    B26Error,", "    Phase1EError as B26Error,")
        # A RootModel union is an evaluated assignment, unlike postponed field
        # annotations. Emit it only after its generated TypedDict dependencies.
        alias = "B26Command = SkillCommand | SharingCommand | RemoteMemoryCommand\n"
        python_source = python_source.replace(alias, "")
        python_source = python_source.replace(
            "class ProtocolNegotiationError", alias + "\n\nclass ProtocolNegotiationError", 1
        )
