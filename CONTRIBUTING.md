...
工作分支通过 PR 合入 `main`，不要求第二位维护者批准。CI 对 PR 和 main 提交运行，
同一 PR 的新提交会取消旧检查；明确的纯文档变更不重复产品测试，未知路径按完整检查处理。
Python 3.10/3.12（含 TUI 定向测试）与 GUI 检查通过后再合并，不强制每次先同步最新 main。
CodeQL 使用默认查询集提供安全结果，不作为额外必需检查；依赖安全修复 PR 不自动合并。

参与交流请遵守 [社区行为准则](CODE_OF_CONDUCT.md)。
