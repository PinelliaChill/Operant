...
`device_id`、`remote_session_id`、`issued_at`、`expires_at`、`nonce`、`expected_version` 和签名。客户端
不确定 Command 是否提交成功时必须先查询原 ID，不得直接重复产生副作用。

### 17.4 配对、身份与权限

- Remote Control 默认关闭，只能由本地用户显式开启；
- 二维码或短时配对码只携带一次性随机数、有效期、Relay/Host 引用和临时校验信息，不携带长期管理 Token；
