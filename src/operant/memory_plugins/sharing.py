...
class SharingService:
    ...
    def _target_git_identity(
        ...
        if not result_artifact_ref or not result_artifact_ref.startswith("git:"):
            raise SharingValidationError("merge result has no verifiable Git commit identity")
    ...
    def _merge_for_evidence(self, evidence: WriterMemoryEvidence, merge_run_id: str) -> Any:
        ...
        if registration.state != "active" or registration.workspace_id != evidence.workspace_id:
            raise SharingPermissionError("Writer project worktree registration is unavailable")
        initialization, registered_root = self._workspace_for_registration(
            evidence.project_id,
            workspace_id=registration.workspace_id,
            workspace_path=None,
        )
        if registration.workspace_ref != f"workspace:{initialization.workspace_hash}":
            raise SharingValidationError("registered workspace identity changed")
        if self._adapter_workspace_path(merge.target_isolation_ref) != registered_root:
            raise SharingValidationError(
                "merge target differs from the registered project workspace"
            )
        if any(
