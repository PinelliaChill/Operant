...
class SQLiteStore:
    ...
    def _downgrade_v17(self, connection: sqlite3.Connection) -> None:
        tables = b25_schema_contracts()[0]
        for table in tables:
            if connection.execute(f'SELECT 1 FROM "{table}" LIMIT 1').fetchone() is not None:
                raise MigrationError("refusing to roll back B2-5 while governance evidence exists")
        for table in tables:
            connection.execute(f'DROP TABLE "{table}"')
