...
当前记忆带有来源、版本、作用域与状态。候选知识经过确认，或满足保守的激活条件后，
才供后续任务使用；更新也不会覆盖历史版本。这样既能积累知识，也能追查一条经验从哪里来、
...
插件化设计进一步面向可替换的记忆实现与其他能力扩展，目标是在不反复改动核心流程的情况下，
组合出适合自己的 Agent 工作环境。
...
| 经验复用 | 记忆的版本、来源、作用域、检索与候选激活 | 完整的自动经验提炼与复用效果评测；目前没有统一的 Token 节省或成功率提升结论 |
| 插件扩展 | 模块边界、MCP 工具接入与受控 Skill 发现 | 完整插件生命周期仍在后续开发分支推进，不能把所有设计能力视为当前可用 |
