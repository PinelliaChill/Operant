...
export const LiveGraphTeamView: React.FC<{ activeTab: LiveTab }> = ({ activeTab }) => {
  ...
  const [knownGraphRuns, setKnownGraphRuns] = useState<Phase23.GraphWorkflowRun[]>([]);
  ...
  const loadDirectory = useCallback(async () => {
    ...
    try {
      const next = normalizeCollaborationDirectory(await b24Client.getCollaborationDirectory());
      ...
      setDirectory(null); setDirectoryError(errorMessage(caught));
  ...
  const loadGraphRuns = useCallback(async () => {
    try {
      const page = await b2Client.listTasks({ limit: 100 });
      const legacyIds = page.items
        .filter((item) => item.source.source_type === 'workflow_run')
        .map((item) => item.source.source_id);
      const projections = await Promise.allSettled(
        legacyIds.map((id) => phase23Client.getGraphRunByLegacyWorkflow(id)),
      );
      const runs = projections
        .filter((result): result is PromiseFulfilledResult<Phase23.GraphWorkflowRun> => result.status === 'fulfilled')
        .map((result) => result.value);
      setKnownGraphRuns((current) => {
        const merged = new Map(current.map((item) => [item.id, item]));
        runs.forEach((item) => merged.set(item.id, item));
        return [...merged.values()].sort((left, right) => right.updated_at.localeCompare(left.updated_at));
      });
    } catch {
      // Graph run discovery is best effort. The formal command receipt remains
      // the source of truth for runs created in this view.
    }
  }, [b2Client, phase23Client]);

  useEffect(() => { void loadDirectory(); void loadAgents(); void loadGraphRuns(); }, [loadAgents, loadDirectory, loadGraphRuns]);
  ...
  const refreshGraph = async (runId = graphRunId, expectedScope = graphScopeEpoch.current, current = () => true) => {
    ...
    setGraphRun(nextRun); setKnownGraphRuns((items) => [nextRun, ...items.filter((item) => item.id !== nextRun.id)]); setNodeRuns(nextNodes);
  ...
  const resolveLegacy = (requested = legacyWorkflowRunId) => {
    ...
    return run(async () => {
      ...
      setGraphRunId(linked.id); setGraphRun(linked); setNodeRuns(nodes); setKnownGraphRuns((items) => [linked, ...items.filter((item) => item.id !== linked.id)]);
  ...
  useLayoutEffect(() => { if (previousWorkspace.current === activeWorkspace) return; previousWorkspace.current = activeWorkspace; workspaceEpoch.current += 1; invalidateGraph({ clearRunId: true, clearEvents: true, cancelPending: true }); clearTeam(true, true); setWorkflowDefinition(null); setKnownGraphRuns([]); }, [activeWorkspace]);
  useEffect(() => () => { streamEpoch.current += 1; graphScopeEpoch.current += 1; graphQueryEpoch.current += 1; teamScopeEpoch.current += 1; teamQueryEpoch.current += 1; busyEpoch.current += 1; safelyReturnIterator(streamIterator.current); streamIterator.current = null; }, []);
